...
if(NOT DEFINED SB_SRC_CLONE_DIR)
    set(SB_SRC_CLONE_DIR ${CMAKE_BINARY_DIR}/src CACHE STRING "The location of the sources retrieved by \"git clone\" or \"svn co\" commands")
endif()
...
#  message("SB_CMAKE_ARGS_${_name} = ${SB_CMAKE_ARGS_${_name}}")

  set(oneValueArgs CVS_REPOSITORY GIT_REPOSITORY SVN_REPOSITORY URL SOURCE_DIR SUBDIR)
  ...
  if (BUILD_${subdir})
    ...
    if(buildFromSourcePackage) # we are building an installed version of the source package
      ...
      set(GET_SOURCES_ARGS ) #DOWNLOAD_DIR ${SB_SRC_CLONE_DIR}/${_name}/ )

      if(_SB_CVS_REPOSITORY)
        set(GET_SOURCES_ARGS ${GET_SOURCES_ARGS} CVS_REPOSITORY ${_SB_CVS_REPOSITORY} SOURCE_DIR ${SB_SRC_CLONE_DIR}/${subdir} )
      ...
      elseif(_SB_SVN_REPOSITORY)
        set(GET_SOURCES_ARGS ${GET_SOURCES_ARGS} SVN_REPOSITORY ${_SB_SVN_REPOSITORY} SOURCE_DIR ${SB_SRC_CLONE_DIR}/${subdir} )
      elseif(_SB_URL)
        set(GET_SOURCES_ARGS ${GET_SOURCES_ARGS} URL ${_SB_URL} SOURCE_DIR ${SB_SRC_CLONE_DIR}/${subdir} )
...
#    externalProject_Add_StepTargets(${_name} package)
    ...
    if(buildFromSourcePackage)
      ...
      install(DIRECTORY ${SB_SRC_CLONE_DIR}/${subdir}  DESTINATION ${SRC_INSTALL_DIR}  COMPONENT ${_name}
              PATTERN .git EXCLUDE
              PATTERN .svn EXCLUDE
              PATTERN CVS EXCLUDE
             )
...
#    add_dependencies(PackageAll ${_name}-package )
    ...
    execute_process(COMMAND ${CMAKE_COMMAND} -E remove_directory ${CMAKE_BINARY_DIR}/build/${subdir}
                    COMMAND ${CMAKE_COMMAND} -E remove_directory ${SB_SRC_CLONE_DIR}/${subdir}
